...
exports.scheduleAMessageRequest = async (req, res) => {
    let {token, date_or_cron_or_rules, startTime, endTime, phone_numbers, body, sendIfTheServerWasDown} = req.body
    ...
    if (status !== false) {
        ...
        return res.status(422).json({ error: 'Error! A message was NOT scheduled!' });
    }
}